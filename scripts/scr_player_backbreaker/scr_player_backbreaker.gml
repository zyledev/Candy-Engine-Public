function scr_player_backbreaker()
{
	...
	if global.combo >= 4 && !global.roomsave
	{
	...
	if animation_end() && sprite_index = spr_player_levelcomplete || sprite_index = spr_playerN_victory
		state = states.normal;
	if animation_end() && sprite_index = spr_bossintro
		state = states.normal;
	if sprite_index = spr_bossintro
	{
		...
		if (global.panic && layer_background_get_sprite(back_id) = 819) // 819
			layer_background_sprite(back_id, 821); // 821
		if (global.panic && layer_background_get_sprite(back_id) = 834) // 834
			layer_background_sprite(back_id, 833); // 833
		if (global.panic && layer_background_get_sprite(back_id) = 831) // 831
			layer_background_sprite(back_id, 832); // 832
		if (global.panic && layer_background_get_sprite(back_id3) = 836) // 836
			layer_background_sprite(back_id3, 837); // 837
		if (global.panic && layer_background_get_sprite(back_id) = 810) // 810
			layer_background_sprite(back_id, 811); // 811
		if (global.panic && layer_background_get_sprite(back_id) = 813) // 813
			layer_background_sprite(back_id, 814); // 814
		if (global.panic && layer_background_get_sprite(back_id2) = 815) // 815
			layer_background_sprite(back_id2, 816); // 816
		if (global.panic && layer_background_get_sprite(back_id) = 857) // 857
			layer_background_sprite(back_id, 858); // 858
		if (global.panic && layer_background_get_sprite(back_id2) = 855) // 855
			layer_background_sprite(back_id2, 856); // 856
		if (global.panic && layer_background_get_sprite(back_id3) = 853) // 853
